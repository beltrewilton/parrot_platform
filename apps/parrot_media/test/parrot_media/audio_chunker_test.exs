defmodule ParrotMedia.AudioChunkerTest do
  ...
  describe "raw audio chunking with chunk_samples" do
    ...
    test "chunks exact-sized buffer", %{state: state} do
      ...
      # 20ms in nanoseconds
      assert new_state.pts == 20_000_000
    end
    ...
    test "accumulates undersized buffer", %{state: state} do
      ...
      # No chunk output, so pts doesn't advance
      assert new_state.pts == 0
    end
    ...
    test "outputs multiple chunks from oversized buffer", %{state: state} do
      ...
      # 80 samples leftover
      assert byte_size(new_state.accumulator) == 160
      # 40ms
    ...
    test "handles multiple small buffers that accumulate to chunks", %{state: state} do
      ...
      assert chunk.pts == 0
      # 64 samples leftover
      assert byte_size(state2.accumulator) == 128
      ...
      assert chunk2.pts == 20_000_000
      # 4 samples leftover
      assert byte_size(state3.accumulator) == 8
    end
    ...
    test "pads final partial chunk on end_of_stream", %{state: state} do
      ...
      # Padded to full chunk size
      assert byte_size(last_buffer.payload) == 1920
  ...
  describe "G.711 chunking with chunk_duration_ms" do
    ...
    test "accumulates and chunks G.711 data correctly", %{state: state} do
      ...
      # Half a chunk leftover
      assert byte_size(new_state.accumulator) == 80
      assert new_state.pts == 40_000_000
    end
  end
  ...
  describe "raw audio chunking with chunk_duration_ms" do
    ...
    test "calculates correct chunk size for duration-based raw audio", %{state: state} do
      ...
      # 10ms
      assert state.chunk_duration_ns == 10_000_000
    end
    ...
    test "chunks stereo audio with duration-based config", %{state: state} do
      ...
      assert chunk.pts == 0
      # 10ms
      assert new_state.pts == 10_000_000
    end
  end
  ...
  describe "timestamp continuity" do
    ...
    test "maintains continuous timestamps across multiple buffers", %{state: state} do
      ...
      buffers = [
        ...
        # 1024 samples (outputs 1 chunk)
        %Buffer{payload: :binary.copy(<<0>>, 2048), pts: nil},
        # 256 samples  
        %Buffer{payload: :binary.copy(<<0>>, 512), pts: nil},
        # 768 samples (outputs 1 chunk)
        %Buffer{payload: :binary.copy(<<0>>, 1536), pts: nil},
        # 1000 samples (outputs 1 chunk)
        %Buffer{payload: :binary.copy(<<0>>, 2000), pts: nil}
      ]
