defmodule ParrotSip.Transport do
  require Logger

  @doc """
  Sends a SIP response message using the UDP transport.

  This function expects the response to be a `%ParrotSip.Message{}` and the source
  ...
  ## Examples
      ...
      ParrotSip.Transport.send_response(response) # if response.source is set

  """
  @spec send_response(ParrotSip.Message.t()) :: :ok | {:error, term()}
  def send_response(%ParrotSip.Message{source: %ParrotSip.Source{} = source} = response) do
    send_response(response, source)
  end

  @spec send_response(ParrotSip.Message.t(), ParrotSip.Source.t()) :: :ok | {:error, term()}
  def send_response(response, source) do
    ParrotSip.Transport.Udp.send_response(response, source)
  end
  ...
  ParrotSip.Serializer and ParrotSip.Deserializer respectively.

  This module provides the pure functional implementation of SIP transport.
  For stateful transport management, see ParrotSip.Transport.StateMachine.
  ...
  alias ParrotSip.Transport.StateMachine, as: TransportSM
  ...
  @doc """
  Delegates serialization of a SIP message to ParrotSip.Serializer.

  ## Examples

      iex> message = ParrotSip.Message.new_request(:invite, "sip:bob@example.com")
      iex> binary = ParrotSip.Transport.serialize(message)
  ...
  @doc """
  Delegates deserialization of a binary SIP message to ParrotSip.Deserializer.
  ...
  ## Examples
      ...
      iex> {:ok, message} = ParrotSip.Transport.deserialize(raw_data, source)
      iex> message.method
      :invite
  """
  ...
  ## Examples
      ...
      iex> prepared = ParrotSip.Transport.prepare_message(message, opts)
      iex> via = ParrotSip.Message.top_via(prepared)
      iex> via.transport
      :udp
  """
  ...
  ## Examples
      ...
      iex> message = ParrotSip.Message.new_request(:invite, "sip:bob@biloxi.com")
      iex> opts = %{type: :udp, local_host: "alice.atlanta.com", local_port: 5060}
      iex> {:ok, conn} = ParrotSip.Transport.determine_connection(message, opts)
      iex> conn.type
      :udp
  """
  ...
  @doc """
  Delegates source info creation to ParrotSip.Deserializer.

  ## Examples

      iex> ParrotSip.Transport.create_source(:udp, "192.168.1.100", 5060, "192.168.1.1", 5060)
  ...
  ## Examples
      ...
      iex> ParrotSip.Transport.send_request(request)
      :ok
  """
  ...
  ## Examples
      ...
      iex> uri = ParrotSip.Transport.local_uri()
      iex> uri.scheme
      "sip"
  """
