  ...
  def handle_info({:packet_received, raw_data, {remote_ip, remote_port}, metadata}, state) do
    ...
    case Parser.parse(raw_data) do
      {:ok, sip_message, _rest} ->
        ...
        Logger.debug("Parsed SIP #{message_with_source.type} #{if message_with_source.type == :request, do: message_with_source.method, else: "#{message_with_source.status} #{message_with_source.reason}"}")
        
        # Route to transaction layer
        route_message(message_with_source, state)
        
      {:error, reason} ->
        Logger.warning("Failed to parse SIP message from #{format_ip(remote_ip)}:#{remote_port}: #{inspect(reason)}")
    end
