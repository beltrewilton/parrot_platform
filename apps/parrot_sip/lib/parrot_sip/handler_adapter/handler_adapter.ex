defmodule ParrotSip.HandlerAdapter do
  @moduledoc """
  ...
  * `user_handler_module` - The module that implements the `ParrotSip.Handler` behaviour
  * `user_handler_state` - The initial state to pass to the user handler's callbacks
  ...
  def new(user_handler_module, user_handler_state) do
    ParrotSip.HandlerAdapter.Core.new(user_handler_module, user_handler_state)
  end
  ...
  * `args` - Arguments to pass to the `start_link/1` function
  ...
  def child_spec(args) do
    ParrotSip.HandlerAdapter.Core.child_spec(args)
  end
  ...
  * `args` - A tuple containing the user's handler module and state
  ...
  def start_link(args) do
    ParrotSip.HandlerAdapter.Core.start_link(args)
  end
end
