defmodule ParrotSip.HandlerAdapter.Core do
  @moduledoc """
  ...
  alias ParrotSip.HandlerAdapter.{
    ResponseHandler,
    CallbackHandler
  }
  ...
  ## Parameters
    ...
    * `user_handler_module` - The module that implements the `ParrotSip.UasHandler` behaviour
  ...
  def new(user_handler_module, user_handler_state) do
    ParrotSip.Handler.new(__MODULE__, {user_handler_module, user_handler_state})
  end
  ...
  # ParrotSip.Handler behaviour callbacks (static functions)
  ...
  def uas_request(uas, req_sip_msg, {user_handler_module, user_handler_state}) do
    ...
    {:ok, pid} =
      ParrotSip.HandlerAdapter.Supervisor.start_child({user_handler_module, user_handler_state})
  ...
  def idle(
        {:call, from},
        {:process_request, uas, %ParrotSip.Message{method: method} = original_req_sip_msg},
        data
      ) do
    Logger.debug(
  ...
  def transaction_trying(
        :cast,
        :progress,
        %ParrotSip.HandlerAdapter.Core.Data{
          handler_module: _handler_module,
          transaction: %ParrotSip.Transaction{request: request}
        } = data
      ) do
    Logger.debug(
