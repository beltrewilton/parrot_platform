  ...
  describe "UAC happy path" do
    test "initiating -> calling -> ringing -> answered -> established", %{notify_fun: notify_fun} do
      ...
      send(
        uac,
        {:tx_response,
         {:response, %Message{status_code: 180, to: %{parameters: %{"tag" => "bob-tag"}}}}}
      )
      ...
      send(
        ...
        {:tx_response,
         {:response,
          %Message{status_code: 200, to: %{parameters: %{"tag" => "bob-tag"}}, body: "v=0\r\n"}}}
      )
  ...
  describe "UAC rejection" do
    ...
    test "call rejected after ringing", %{notify_fun: notify_fun} do
      ...
      send(
        uac,
        {:tx_response,
         {:response, %Message{status_code: 480, to: %{parameters: %{"tag" => "bob-tag"}}}}}
      )
  ...
  describe "UAC progress responses" do
    test "multiple 18x responses", %{notify_fun: notify_fun} do
      ...
      send(
        uac,
        {:tx_response,
         {:response, %Message{status_code: 183, to: %{parameters: %{"tag" => "bob-tag"}}}}}
      )
