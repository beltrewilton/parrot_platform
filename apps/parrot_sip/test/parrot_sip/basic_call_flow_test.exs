defmodule ParrotSip.BasicCallFlowTest do
  ...
  test "INVITE -> 200 OK -> ACK -> BYE flow completes successfully" do
    ...
    # Wait for INVITE to be processed and get the generated To-tag from the 200 OK response
    assert_receive {:invite_received, _msg}, 1000
    assert_receive {:response_sent, response}, 1000

    # Extract the To-tag that was generated by the UAS layer
    to_tag = response.to.parameters["tag"]
    assert to_tag != nil, "200 OK response should have a To-tag"

    # Now send ACK with the correct To-tag from the 200 OK
    ack_msg =
      "ACK sip:service@127.0.0.1:5060 SIP/2.0\r\n" <>
        "Via: SIP/2.0/UDP 127.0.0.1:5080\r\n" <>
        "From: sipp <sip:sipp@127.0.0.1:5080>;tag=1\r\n" <>
        "To: sut <sip:service@127.0.0.1:5060>;tag=#{to_tag}\r\n" <>
        "Call-ID: test-call-123\r\n" <>
        "Cseq: 1 ACK\r\n" <>
        "Contact: sip:sipp@127.0.0.1:5080\r\n" <>
        "Content-Length: 0\r\n" <>
    ...
    # Now send BYE with the correct To-tag
    bye_msg =
  ...
  defmodule TestHandler do
    ...
    def handle_invite(uas, sip_msg, args) do
      ...
      # Send the response to the transaction layer which will add the To-tag
      {:ok, final_response} = Server.response(response, uas)

      # Send the final response (with To-tag) back to the test process
      send(args.test_pid, {:response_sent, final_response})
      :ok
    end
