defmodule ParrotExampleUas do
  ...
  def start(opts \\ []) do
    ...
    handler =
      Parrot.Sip.Handler.new(
        ...
        {__MODULE__, %{calls: %{}}},
        # Only show info and above from transport
        log_level: :info,
        # But always show full SIP messages for debugging
        sip_trace: true
      )
  ...
  defp start_media_for_dialog(dialog_id) do
    with [{_pid, {media_session_id, media_pid}}] <-
           Registry.lookup(Parrot.Registry, {:my_app_media, dialog_id.call_id}) do
